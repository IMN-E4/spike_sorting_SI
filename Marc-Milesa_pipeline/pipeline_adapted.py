...
########### Functions ###########
def prepare_recs(raw_files, indx_to_slice=range(32), fs=20_000., dtype='uint16', n_channels=32, dist_contacts=40, gain_val=0.3815):
    ...
    # Read raw recording
    rec_raw = si.read_binary(raw_files, dtype=dtype, num_chan=n_channels, sampling_frequency=fs, gain_to_uV=gain_val, offset_to_uV=offset_to_uv)
...
def run_spike_sorting(files):
    ...
    for group , rec_one_channel in splitted_rec.items():
        ...
        rec_preprocess_one_seg_saved = rec_preprocess_one_seg.save(folder=tmp_path, n_jobs=10) # Saving was necessary to correct it for sorters
        print(rec_preprocess_one_seg_saved)
        
        # Start sorting
        for sorter_name, params in sorters.items():
            sorting_folder =  working_folder / f"contact_{group}"/ f"sorting_{sorter_name}"
            ...
            if sorting_folder.exists():
                ...
                try:
                    ...
                    sorting_one_seg = si.run_sorter(
                        ...
                        output_folder= working_folder / f"contact_{group}" / f"raw_sorting_{sorter_name}",
            ...
            sorting_save_path = working_folder / f"contact_{group}" / f"sorting_{sorter_name}"
            ...
            if sorting_save_path.exists():
                ...
                sorting = sorting.save(
                    format="npz", folder=working_folder / f"contact_{group}" / f"sorting_{sorter_name}" 
                    )
            ...
            si.compute_correlograms(
                we, window_ms=50.0, bin_ms=1.0, load_if_exists=True
            )

            # report_folder = working_folder / f"contact_{group}" / f"report_clean_{sorter_name}"
