...
recording_spike = si.read_spikeglx(data_folder, stream_id='imec0.ap') # ap file
#print(recording)

recording_spike = recording_spike.channel_slice(channel_ids=recording_spike.channel_ids[:10])
...
# Spike source
# spike_source = ephyviewer.FromSpikeinterfaceSorintgSource(sorting)


# Filtering
recording_f = si.bandpass_filter(recording_spike)
recording_f = si.common_reference(recording_f, reference='local', local_radius=(50, 100))


# Sources
# TODO ephyviewer.SpikeInterfaceRecordingSource
...
# view2 = ephyviewer.TraceViewer(source=sig_source2, name='signals nidq') # Trace of Microphone
# win.add_view(view2)
...
for channel in range(recording_spike.get_num_channels()):
   ...
   for view in (view0, view3):
        view.by_channel_params[f'ch{channel}', 'visible'] = (channel <10)
        # if channel <3:
